...
(function($) {
    ...
    var methods = {
      init: function (options) {
        ...
        isOpen: false,
        badgeElement: false,
        serverApiGateway: "/server/node/server.js",
        ...
        defaultUserImg: '/img/user-icon-small.jpg'
            ...
            if (typeof dayjs == 'undefined') methods.validationError('DayJS is not available. Get it from https://day.js.org/');
            ...
            $(this).on('click', function () {
                ...
                $('.slack-message-box').height($('.slack-chat-box').height() - $('.desc').height() - $('.send-area').height() - parseInt(window.slackChat._options.heightOffset,10));
        ...
        querySlack: function ($elem) {
            var options = window.slackChat._options;
            ...
            methods.createChannel($elem, function (channel) {
                ...
                $.ajax({
                            ...
                            for (var i=0; i< resp.messages.length; i++)
                            {
                                ...
                                if (resp.messages[i].subtype == 'bot_message' && resp.messages[i].text !== "") {
                                    ...
                                    if (typeof dayjs !== 'undefined')
                                        html += "<div class='timestamp'>" + dayjs.unix(resp.messages[i].ts).fromNow() + "</div>";
                                    html += "</div>";
                                    html += "</div>";
                                }
                  ...
                  userName = options.defaultSysUser;
                  userImg = options.defaultSysImg;
        ...
        postMessage: function ($elem) {
            ...
            var message = $('.slack-new-message').val();
